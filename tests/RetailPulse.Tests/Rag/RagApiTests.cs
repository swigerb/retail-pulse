...
public class RagApiTests
{
    ...
    public async Task Upload_ValidContent_ReturnsDocumentId()
    {
        ...
        string docId = await kb.IngestDocumentAsync("Test Document",
    ...
    public async Task Upload_LargeContent_ProducesMultipleChunks()
    {
        ...
        string docId = await kb.IngestDocumentAsync("Large Doc", largeContent, "upload");
    ...
    public async Task Search_RespectTopK()
    {
        ...
        IReadOnlyList<SearchResult> results = await kb.SearchAsync("retail strategy", topK: 2);
    ...
    {
        InMemoryKnowledgeBase kb = CreateKb();
        string docId = await kb.IngestDocumentAsync("To Delete", "Temporary content.", "src");
        kb.DocumentCount.Should().Be(1);
    ...
    public async Task Delete_PreservesOtherDocuments()
    {
        ...
        await kb.IngestDocumentAsync("Keep",
            ...
            "src");
        string id2 = await kb.IngestDocumentAsync("Remove",
    ...
    public async Task RagContextProvider_WithMatchingContent_ReturnsContext()
    {
        ...
        string? context = await provider.GetContextAsync("How does dynamic pricing work?");
    ...
    public async Task RagContextProvider_EmptyKB_ReturnsNull()
    {
        ...
        string? context = await provider.GetContextAsync("Any question");
