...
public class KnowledgeBaseTests
{
    ...
    public async Task Ingest_SingleDocument_IncreasesDocumentCount()
    {
        ...
        string docId = await kb.IngestDocumentAsync("Holiday Planning Guide",
    ...
    public async Task Ingest_ReturnsUniqueDocumentIds()
    {
        ...
        string id1 = await kb.IngestDocumentAsync("Doc A", "Content A", "src");
        string id2 = await kb.IngestDocumentAsync("Doc B", "Content B", "src");
    ...
    public async Task Search_IrrelevantQuery_ReturnsEmpty()
    {
        ...
        IReadOnlyList<SearchResult> results = await kb.SearchAsync("quantum computing algorithms");
    ...
    public async Task Search_ReturnsScores()
    {
        ...
        IReadOnlyList<SearchResult> results = await kb.SearchAsync("pricing");
    ...
    public async Task Search_TopK_LimitsOutput()
    {
        ...
        IReadOnlyList<SearchResult> results = await kb.SearchAsync("retail", topK: 3);
    ...
    {
        InMemoryKnowledgeBase kb = CreateKb();
        string docId = await kb.IngestDocumentAsync("Temporary Doc", "This document will be deleted.", "src");
    ...
    {
        InMemoryKnowledgeBase kb = CreateKb();
        string id1 = await kb.IngestDocumentAsync("Doc A",
        ...
        IReadOnlyList<SearchResult> results = await kb.SearchAsync("marketing");
        results.Should().NotBeEmpty();
    }
    ...
    public async Task Delete_NonexistentId_DoesNotThrow()
    {
        ...
        Func<Task> act = async () => await kb.DeleteDocumentAsync("nonexistent-id-12345");
    ...
    public async Task ConcurrentIngestion_DoesNotCorruptState()
    {
        ...
        IEnumerable<Task<string>> tasks = Enumerable.Range(0, 20).Select(i =>
