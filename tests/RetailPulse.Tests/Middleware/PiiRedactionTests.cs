...
public class PiiRedactionTests
{
    ...
    public void Redact_Ssn_ReplacedWithPlaceholder(string input, string expected) => PiiRedactor.Redact(input).Should().Be(expected);

    [Fact]
    public void Redact_SsnWithSpaces_ReplacedWithPlaceholder() => PiiRedactor.Redact("SSN is 123 45 6789").Should().Be("SSN is [REDACTED:ssn]");
    ...
    public void Redact_Email_ReplacedWithPlaceholder(string input, string expected) => PiiRedactor.Redact(input).Should().Be(expected);

    [Fact]
    public void Redact_EmailWithSubdomain_Redacted()
    {
        string result = PiiRedactor.Redact("Send to user@mail.example.co.uk");
    ...
    public void Redact_Phone_ReplacedWithPlaceholder(string input, string expected) => PiiRedactor.Redact(input).Should().Be(expected);

    [Fact]
    public void Redact_PhoneWithDots_Redacted()
    {
        string result = PiiRedactor.Redact("Dial 555.123.4567 for support");
    ...
    {
        string result = PiiRedactor.Redact("Card: 4111111111111111");
        result.Should().Be("Card: [REDACTED:credit_card]");
    }
    ...
    {
        string input = "Contact user@example.com or call (555) 123-4567. SSN: 123-45-6789.";
    ...
    {
        string input = "Emails: alice@corp.com and bob@corp.com";
        string result = PiiRedactor.Redact(input);
    ...
    public void Redact_NoPii_ResponseUnchanged(string input) => PiiRedactor.Redact(input).Should().Be(input);

    [Fact]
    public void Redact_EmptyString_ReturnsEmpty() => PiiRedactor.Redact("").Should().BeEmpty();

    [Fact]
    public void Redact_Null_ReturnsNull() => PiiRedactor.Redact(null!).Should().BeNull();
    ...
    public void Redact_ProductCode_NotRedacted()
    {
        ...
        string input = "Product SKU: ABC-12-345";
    ...
    public void Redact_ShortNumber_NotRedacted()
    {
        ...
        string input = "Order #12345 is confirmed";
        PiiRedactor.Redact(input).Should().Be(input);
    }
    ...
    {
        string input = "Growth was 12-15% year over year";
        PiiRedactor.Redact(input).Should().Be(input);
    }
    ...
    {
        string input = "Report date: 01/15/2026";
        PiiRedactor.Redact(input).Should().Be(input);
    }
    ...
    [Fact]
    public void ContainsPii_WithSsn_ReturnsTrue() => PiiRedactor.ContainsPii("My SSN is 123-45-6789").Should().BeTrue();

    [Fact]
    public void ContainsPii_NoPii_ReturnsFalse() => PiiRedactor.ContainsPii("Normal text about sales").Should().BeFalse();

    [Fact]
    public void ContainsPii_Empty_ReturnsFalse() => PiiRedactor.ContainsPii("").Should().BeFalse();
