...
public class AuditLogTests
{
    ...
    {
        AuditEntry entry = MakeEntry("e1", agentId: "agent-1", userId: "user-1", action: "chat");
        await _log.LogAsync(entry);

        IReadOnlyList<AuditEntry> results = await _log.QueryAsync(new AuditQuery());
    ...
    public async Task QueryByAgentId_FiltersCorrectly()
    {
        ...
        IReadOnlyList<AuditEntry> results = await _log.QueryAsync(new AuditQuery(AgentId: "agent-1"));
    ...
    public async Task QueryByUserId_FiltersCorrectly()
    {
        ...
        IReadOnlyList<AuditEntry> results = await _log.QueryAsync(new AuditQuery(UserId: "alice"));
    ...
    public async Task QueryByAction_FiltersCorrectly()
    {
        ...
        IReadOnlyList<AuditEntry> results = await _log.QueryAsync(new AuditQuery(Action: "chat"));
    ...
    public async Task QueryLimit_IsRespected()
    {
        ...
        IReadOnlyList<AuditEntry> results = await _log.QueryAsync(new AuditQuery(Limit: 5));
    ...
    public async Task RingBuffer_EvictsOldest_Beyond5000Entries()
    {
        ...
        IReadOnlyList<AuditEntry> all = await _log.QueryAsync(new AuditQuery(Limit: 6000));
    ...
    public async Task RingBuffer_NewestEntriesPreserved()
    {
        ...
        IReadOnlyList<AuditEntry> latest = await _log.QueryAsync(new AuditQuery(Limit: 1));
    ...
    public async Task Stats_CountByAgent_Accurate()
    {
        ...
        AuditStats stats = await _log.GetStatsAsync();
