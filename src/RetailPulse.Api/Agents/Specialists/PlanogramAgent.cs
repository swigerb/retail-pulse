...
public class PlanogramAgent : ISpecialistAgent
{
    ...
    private readonly ILogger<PlanogramAgent> _logger;
    ...
    public async Task<ChatResponse> HandleAsync(ChatRequest request, CancellationToken ct = default)
    {
        ...
        catch (ClientResultException ex) when (ex.Status == 429)
        {
            ...
            _logger.LogWarning(ex,
                "Planogram agent rate-limited after {DurationMs}ms. Status: {Status}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex,
                "Planogram agent failed after {DurationMs}ms for session {SessionId}",
                failureDurationMs, sessionId);

            return new ChatResponse(
                "⚠️ Something went wrong while optimizing the planogram. Please try again.",
                sessionId, [], null, failureDurationMs);
        }
        ...
        var reply = response.Text ?? "I wasn't able to generate a planogram optimization response.";
        ...
        _logger.LogInformation("Planogram agent responded in {DurationMs}ms with {SpanCount} spans, {ChartCount} charts, {TokenCount} tokens",
