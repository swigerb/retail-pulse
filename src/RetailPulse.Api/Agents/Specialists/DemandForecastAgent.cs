...
public class DemandForecastAgent : ISpecialistAgent
{
    ...
    private readonly ILogger<DemandForecastAgent> _logger;
    ...
    public DemandForecastAgent(
        ...
        ILogger<DemandForecastAgent> logger,
    ...
    public async Task<ChatResponse> HandleAsync(ChatRequest request, CancellationToken ct = default)
    {
        ...
        catch (ClientResultException ex) when (ex.Status == 429)
        {
            ...
            _logger.LogWarning(ex,
                "Demand forecast agent rate-limited after {DurationMs}ms. Status: {Status}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex,
                "Demand forecast agent failed after {DurationMs}ms for session {SessionId}",
                failureDurationMs, sessionId);

            return new ChatResponse(
                "⚠️ Something went wrong while generating the demand forecast. Please try again.",
                sessionId, [], null, failureDurationMs);
        }
        ...
        var reply = response.Text ?? "I wasn't able to generate a forecast response.";
        ...
        _logger.LogInformation("Demand forecast agent responded in {DurationMs}ms with {SpanCount} spans, {ChartCount} charts, {TokenCount} tokens",
