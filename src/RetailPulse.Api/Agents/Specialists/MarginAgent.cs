...
public class MarginAgent : ISpecialistAgent
{
    ...
    private readonly ILogger<MarginAgent> _logger;
    ...
    public async Task<ChatResponse> HandleAsync(ChatRequest request, CancellationToken ct = default)
    {
        ...
        catch (ClientResultException ex) when (ex.Status == 429)
        {
            ...
            _logger.LogWarning(ex,
                "Margin analysis agent rate-limited after {DurationMs}ms. Status: {Status}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex,
                "Margin analysis agent failed after {DurationMs}ms for session {SessionId}",
                failureDurationMs, sessionId);

            return new ChatResponse(
                "⚠️ Something went wrong while analyzing margins. Please try again.",
                sessionId, [], null, failureDurationMs);
        }
        ...
        var reply = response.Text ?? "I wasn't able to generate a margin analysis response.";
        ...
        _logger.LogInformation("Margin analysis agent responded in {DurationMs}ms with {SpanCount} spans, {ChartCount} charts, {TokenCount} tokens",
