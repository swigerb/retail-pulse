...
public class StoreOpsAgent : ISpecialistAgent
{
    ...
    private readonly ILogger<StoreOpsAgent> _logger;
    ...
    public async Task<ChatResponse> HandleAsync(ChatRequest request, CancellationToken ct = default)
    {
        ...
        catch (ClientResultException ex) when (ex.Status == 429)
        {
            ...
            _logger.LogWarning(ex,
                "Store ops agent rate-limited after {DurationMs}ms. Status: {Status}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex,
                "Store ops agent failed after {DurationMs}ms for session {SessionId}",
                failureDurationMs, sessionId);

            return new ChatResponse(
                "⚠️ Something went wrong while analyzing store operations. Please try again.",
                sessionId, [], null, failureDurationMs);
        }
        ...
        var reply = response.Text ?? "I wasn't able to generate a store operations response.";
        ...
        _logger.LogInformation("Store ops agent responded in {DurationMs}ms with {SpanCount} spans, {ChartCount} charts, {TokenCount} tokens",
