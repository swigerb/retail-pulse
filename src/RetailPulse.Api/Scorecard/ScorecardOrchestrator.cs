...
public class ScorecardOrchestrator
{
        ...
        string[] brands, string? region = null, CancellationToken ct = default)
    {
        ...
        var brandTasks = brands.Select(b => ScoreBrandAsync(b, region, ct));
        var brandScores = await Task.WhenAll(brandTasks);

        // Sort by overall score descending
        var sorted = brandScores.OrderByDescending(b => b.OverallScore).ToArray();

        // Generate executive summary
        var execSummary = await GenerateExecSummaryAsync(sorted, region, ct);

        var topActions = sorted
        ...
        string brand, string? region, CancellationToken ct)
    {
        ...
        var dimTasks = ScoringDimensions.Select(async dim =>
        {
            ...
            try
            {
                ...
                var response = await agent.HandleAsync(request, cts.Token);
        ...
        var dimResults = await Task.WhenAll(dimTasks);

        foreach (var dim in dimResults)
            dimensions[dim.Dimension] = dim;

        var overallScore = dimResults.Sum(d => d.WeightedScore);

        var summary = overallScore >= 7.5 ? $"{brand} is performing strongly across most dimensions."
    ...
    {
        var regionClause = string.IsNullOrWhiteSpace(region) ? "across all regions" : $"in {region}";
        return $$"""
        ...
        string dimension, double weight, string agentKey, string responseText)
    {
        try
        {
            ...
            if (jsonStart >= 0 && jsonEnd > jsonStart)
            {
                ...
                var assessment = root.TryGetProperty("assessment", out var assessEl)
    ...
    {
        var brandSummaries = string.Join("\n", brands.Select(b =>
