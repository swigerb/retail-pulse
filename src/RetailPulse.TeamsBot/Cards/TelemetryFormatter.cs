...
public static class TelemetryFormatter
{
    ...
    /// </summary>
    public static string FormatDuration(double durationMs) => durationMs >= 1000 ? $"{durationMs / 1000:F1}s" : $"{durationMs:F0}ms";
    ...
    /// </summary>
    public static string TruncateName(string name, int maxLength = 30) => string.IsNullOrEmpty(name) || name.Length <= maxLength ? name ?? string.Empty : string.Concat(name.AsSpan(0, maxLength - 1), "…");
    ...
    public static int CalculateWaterfallWidth(double spanDuration, double maxDuration)
    {
        ...
        double percentage = spanDuration / maxDuration * 100.0;
