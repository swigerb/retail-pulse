...
### 2026-08-07T14:49:59-04:00: Tool-Context Budget stops catastrophic token amplification (P0)

**By:** Retail Pulse Squad (owner), Kroger / Costco / Publix charters

**Status:** Delivered for parent review on branch `squad/44-optimize-tool-context` (issue #44). NOT merged/deployed — parent independent-reviews, merges, deploys, and runs the live authenticated verification.

**Problem:** The exact query `Compare Coastline Tacos vs Apex Grill depletions across all regions` cost 437,120 tokens (436,648 input) and 43.2s with 3 tool calls. Root cause: unbounded `weekly_data` arrays from `GetHistoricalDemand` (and other raw tool payloads) entered model context in full, were re-sent every iteration (`MaximumIterationsPerRequest`), and were duplicated by prefetch injection.

**Fix:** A centralized, typed `ToolResultBudget` compaction boundary wired at the single `AgentExecutionPipeline` tool-wrap choke point (covers every specialist agent):
- Tool-specific summarizers (`HistoricalDemand` -> faithful `by_region` rollups; `PortfolioDepletion` -> compact per-brand metrics), generic array-compaction fallback with explicit truncation metadata, and a valid-JSON hard-clip last resort — never malformed JSON or silent data loss.
- Request-scoped dedup (AsyncLocal, keyed by normalized tool name+args, per session/principal) so identical repeated calls reuse the compact result; distinct brand calls stay distinct.
- Per-result, cumulative, and tool-call-count caps with actionable diagnostics instead of context explosion.
- Prefetch payloads compacted before system-prompt injection (no double-inject).
- `CreateChart` exempt: canonical `ChartSpec` passes through byte-for-byte and does not count against the budget, so frontend chart data stays complete.
- Telemetry records sizes/items/estimated-tokens/flags only — no payload contents or PII.

**Deterministic before/after** (single-occurrence est. tokens): 74,868 -> 1,412 = **98.1% reduction**. `GetHistoricalDemand` 36,851 -> 275 and 35,668 -> 274; `PortfolioDepletion` 2,012 -> 526. Two-brand grouped-bar fixture preserved (2 series / 6 marks), exempt from compaction.

**Validation:** Release build 0W/0E; backend suite 2,499 passed / 0 failed (2 LoadTests skipped); frontend `npm ci` via proxy + `npm run build` passed. 38 new Budget tests plus a catalog contract gate that fails CI if a new unbounded agent tool is added.

**Team impact:**
- **Backend / Costco:** All tool results must pass the `ToolResultBudget` boundary; new tools need an explicit bounded-output classification (Exempt / ToolSpecificSummarizer / GenericBudget) or the catalog contract test fails CI.
- **QA / Publix:** Token/size budgets are now primary CI gates; the two-brand fixture asserts both chart correctness and compact tool-context.
- **Lead / Kroger:** Detailed drill-down remains available opt-in; canonical ChartSpec is never compacted.